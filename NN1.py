...
df_fe = list_features[0]
df_PT = list_PT_index[0]
df_fe['Marker'] = np.nan
for j in range(len(df_PT)):
    for k in range(len(df_fe)):
        if df_fe.Combination[k] == df_PT.Index[j]:
            df_fe.Marker[k] = df_PT.PT[j]
list_fe = df_fe.values.tolist()

for i in range(1,len(list_features)):
    df_fe2 = list_features[i]
    df_PT2 = list_PT_index[i]
    df_fe2['Marker'] = np.nan
    for j in range(len(df_PT2)):
        for k in range(len(df_fe2)):
            if df_fe2.Combination[k] == df_PT2.Index[j]:
                df_fe2.Marker[k] = df_PT2.PT[j]
    list_fe2 = df_fe2.values.tolist()
    
    for list_f in list_fe2:
        list_fe.append(list_f)


df_features_total = pd.DataFrame()
df_features_total[['Sum', 'Max', 'Min', 'Combination', 'Common_side','Marker']] = list_fe

df_features_total = df_features_total.drop(['Combination', 'Common_side'], axis = 1)
df_features_total.Sum = pd.to_numeric(df_features_total.Sum)
df_features_total.Max = pd.to_numeric(df_features_total.Max)
df_features_total.Min = pd.to_numeric(df_features_total.Min)
df_features_total = df_features_total.fillna(-1)

df_features_pt = df_features_total[df_features_total['Marker'] != -1]
df_features_na = df_features_total[df_features_total['Marker'] == -1]
df_features_na2 = df_features_na.sample(n = len(df_features_pt))

df_total = pd.concat([df_features_pt, df_features_na2])
...
train_x = df_total.drop(['Marker'], axis = 1)
train_y = df_total['Marker']

X_train, X_test, Y_train, Y_test = train_test_split(train_x, train_y, random_state = 1)

mlp = MLPClassifier(solver = 'lbfgs', activation = 'tanh', random_state = 1, max_iter = 100000).fit(X_train, Y_train)

print("mlp_training_model_evlouaton："+str(accuracy_score(Y_train,mlp.predict(X_train))))
print("mlp_testing_model_evlouation："+str(accuracy_score(Y_test,mlp.predict(X_test))))
...
mlp = MLPClassifier(solver = 'lbfgs', activation = 'tanh', random_state = 1, max_iter = 100000).fit(X_train_scald, Y_train)
print("mlp_training_model_evlouaton："+str(accuracy_score(Y_train,mlp.predict(X_train))))
print("mlp_testing_model_evlouation："+str(accuracy_score(Y_test,mlp.predict(X_test))))
